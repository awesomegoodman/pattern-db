 ...
 *
 * Outputs: company counts by domain + evidence weight, pattern coverage,
...
async function run() {
  ...
  // ── Companies by domain ─────────────────────────────────────────────────────
  console.log("── Companies by domain ──────────────────────────");
  const byDomain = await db.unsafe(\`
    SELECT
      domain,
      COUNT(*)                                                          AS total,
      COUNT(*) FILTER (WHERE evidence_weight = 'strong_validator')      AS strong,
      COUNT(*) FILTER (WHERE evidence_weight = 'weak_validator')        AS weak,
      COUNT(*) FILTER (WHERE evidence_weight = 'disconfirming')         AS fail,
      COUNT(*) FILTER (WHERE evidence_weight = 'unknown')               AS unknown
    FROM companies
    GROUP BY domain
    ORDER BY total DESC
  \`);
  const maxD = Math.max(...byDomain.map((r: any) => Number(r.total)));
  for (const r of byDomain) {
    console.log(\`  \${String(r.domain).padEnd(20)} \${bar(Number(r.total), maxD)} \${r.total} (✓\${r.strong} ·\${r.fail}✗)\`);
  }
